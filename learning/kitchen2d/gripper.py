...
def create_gripper(world, lgripper_pos, rgripper_pos, 
    ...
    pj = world.CreatePrismaticJoint(
        ...
        upperTranslation=s.OPEN_WIDTH - init_width,
        enableLimit=True,
...
class Gripper(object):
    ...
    def grasp_translation(self, body, pos_ratio):
        ...
        if body.userData == 'spoon' or body.userData == 'stir':
            ...
            # print(body.angle)
            assert(np.isclose(body.angle, 0, atol=5e-3))
    ...
    def is_graspable(self, body):
        ...
        '''
        return body.usr_w <= s.OPEN_WIDTH
    ...
    def set_open(self):
        ...
        '''
        width = s.OPEN_WIDTH + self.gripper_width
        self.lgripper.position, self.rgripper.position = get_gripper_lrpos(self.position, self.angle, width)
        self.pj.motorSpeed = MOTOR_SPEED
    ...
    def push(self, body, rel_pos, goal_x, maxspeed):
        ...
        while posa_metric(self.position, self.angle, dpos, dangle) > 0.01:
            self.apply_lowlevel_control(dpos, dangle, maxspeed=MAX_SPEED)
            adjust_cnt += 1
            if adjust_cnt > 10:
                break
    ...
    def apply_lowlevel_control(self, dpos, dangle, maxspeed=MAX_SPEED, collision_check=False):
        '''
        ...
        for step in range(len(ddy)):
            ...
            rforce -= g_aforce
            self.lgripper.ApplyForce(lforce, self.lgripper.position, wake=True)
            assert (self.lgripper.position[1] < 25)
            self.rgripper.ApplyForce(rforce, self.rgripper.position, wake=True)
            self.b2w.step()
    ...
    def check_path_collision(self, pos, angle, dpos, dangle):
        ...
        '''
        self.set_position(pos, angle)
        traj, collision = self.apply_lowlevel_control(dpos, dangle, maxspeed=MAX_SPEED, collision_check=True)
        if not collision:
    ...
    def apply_control(self, path, maxspeed=MAX_SPEED):
        '''
        Apply the lowlevel controller to follow a path.
        '''
        for q in path:
            self.apply_lowlevel_control(q[:2], q[2], maxspeed=MAX_SPEED)
    ...
    def find_path(self, dpos, dangle, collision_buffer=None, motion_angle=0, maxspeed=MAX_SPEED):
        '''
        ...
        if self.planning:
            ...
            self.apply_control(traj, maxspeed=MAX_SPEED)
        adjust_cnt = 0
