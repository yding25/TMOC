...
class Kitchen2D(b2WorldInterface):
    def __init__(self, do_gui, sink_w, sink_h, sink_d, sink_pos_x, 
        ...
        self.base = world.CreateStaticBody(
            ...
            shapes=[b2PolygonShape(vertices=[(-left_table_width, 0),
                                             (-left_table_width, sink_d),
                                             (right_table_width, 0),
                                             (right_table_width, sink_d)])],
            userData='countertop'
        )
    ...
    #     '''
    #     if (not self.planning) and (not self.disable_liquid):
...
def make_body(kitchen, name, pose, args):
    ...
    if 'gripper' in name:
        ...
        body = make_circle(kitchen, pose[:2], pose[2], *args[name])
    else:
...
def make_circle(b2world_interface, pos,  w, h=None, d=None, shifth=0.0):
    '''
